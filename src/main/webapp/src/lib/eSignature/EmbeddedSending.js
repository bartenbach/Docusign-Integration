 ...
 */
 ...
 //, eg002 = require('./eSigningViaEmail') // used to create envelope
 ;

const EmbeddedSending = exports
...
*/
EmbeddedSending.createController = async (req, res) => {
...
*/
// ***DS.snippet.0.start
EmbeddedSending.worker = async (args) => {
...
*/
EmbeddedSending.getController = (req, res) => {
 // Check that the authentication token is ok with a long buffer time.
 // If needed, now is the best time to ask the user to authenticate
 // since they have not yet entered any information into the form.
 let tokenOK = req.dsAuth.checkToken();
 if (tokenOK) {
     res.render('components/EmbeddedSending', {
